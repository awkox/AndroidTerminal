...
void apply_pending_resize(native_ssh* h) {
    ...
    if (h->channel != nullptr) {
        ...
        // kill/connect_fail 后 channel 可能已被 free_channel 并发释放（它与本
        // 调用同持 io_mutex，串行化），且 killed 置位意味着会话正在终止，此时
        // 再下发尺寸已无意义。必须锁内复查，避免对已释放 channel 调用。
        if (h->killed || h->channel == nullptr) {
...
void free_channel(native_ssh* h) {
    ...
    // pty_ready 之后 App 线程（sshResize/apply_pending_resize）可能正并发持有
    // io_mutex 执行 change_pty_size，释放必须与 io_reader/io_writer/sshResize
    // 同一把锁内进行，否则构成 UAF。
    pthread_mutex_lock(&h->io_mutex);
