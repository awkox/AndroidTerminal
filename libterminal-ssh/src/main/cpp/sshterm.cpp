...
std::string ssh_error_str(ssh_session session, const char* prefix) {
    std::string msg(prefix);
    msg += ": ";
    const char* detail = session != nullptr ? ssh_get_error(session) : "null session";
    msg += detail != nullptr ? detail : "unknown error";
    return msg;
}

void throw_ssh_exception(JNIEnv* env, const char* class_name,
    ...
    jobject ex = ctor != nullptr ? env->NewObject(clazz, ctor, jmsg) : nullptr;
    if (ex != nullptr) {
        env->Throw(static_cast<jthrowable>(ex));
    }
}

void throw_hostkey_mismatch(JNIEnv* env, const std::string& got,
                            const std::string& expected) {
    const std::string msg = "host key verification failed: got " + got +
        ", expected " + expected;
    jclass clazz = env->FindClass(
        "com/awkoo/terminal/core/SshHostKeyMismatchException");
    if (clazz == nullptr) {
        return;
    }
    jstring jmsg = env->NewStringUTF(msg.c_str());
    jstring jgot = env->NewStringUTF(got.c_str());
    jstring jexpected = env->NewStringUTF(expected.c_str());
    if (jmsg == nullptr || jgot == nullptr || jexpected == nullptr) {
        return;
    }
    jmethodID ctor = env->GetMethodID(
        clazz, "<init>", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
    jobject ex = ctor != nullptr
        ? env->NewObject(clazz, ctor, jmsg, jgot, jexpected)
        : nullptr;
    if (ex != nullptr) {
        env->Throw(static_cast<jthrowable>(ex));
    }
...
Java_com_awkoo_libterminal_ssh_SshProcess_sshConnect(
    JNIEnv* env,
    jclass,
    jstring jhost,
    jint jport,
    ...
    if (native_fd < 0) {
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            "invalid native fd");
        return 0;
    }
    ...
    if (ssh_options_set(session, SSH_OPTIONS_HOST, host.c_str()) != SSH_OK ||
        ...
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            ssh_error_str(session, "set options"));
        ssh_free(session);
        ::close(native_fd);
        return 0;
    }
    ...
    if (server_fingerprint.empty()) {
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            ssh_error_str(session, "obtain server host key"));
    ...
    if (!expected_fingerprint.empty() && server_fingerprint != expected_fingerprint) {
        throw_hostkey_mismatch(env, server_fingerprint, expected_fingerprint);
        ssh_disconnect(session);
        ssh_free(session);
        ::close(native_fd);
        return 0;
    ...
    if (!privkey_path.empty()) {
        ...
        if (ssh_pki_import_privkey_file(privkey_path.c_str(), passphrase.c_str(),
            ...
            privkey == nullptr) {
            throw_ssh_exception(env, "com/awkoo/terminal/core/SshAuthException",
                            ssh_error_str(session, "load private key"));
            ssh_disconnect(session);
            ssh_free(session);
            ::close(native_fd);
            return 0;
        ...
        if (rc != SSH_AUTH_SUCCESS) {
            throw_ssh_exception(env, "com/awkoo/terminal/core/SshAuthException",
                            ssh_error_str(session, "public key auth"));
            ssh_disconnect(session);
            ssh_free(session);
            ::close(native_fd);
            return 0;
        ...
        if (ssh_userauth_password(session, user.c_str(), password.c_str()) !=
            SSH_AUTH_SUCCESS) {
            throw_ssh_exception(env, "com/awkoo/terminal/core/SshAuthException",
                            ssh_error_str(session, "password auth"));
            ssh_disconnect(session);
            ssh_free(session);
            ::close(native_fd);
            return 0;
    ...
    if (channel == nullptr) {
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            ssh_error_str(session, "new channel"));
        ssh_disconnect(session);
        ssh_free(session);
        ::close(native_fd);
        return 0;
    ...
    if (ssh_channel_open_session(channel) != SSH_OK) {
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            ssh_error_str(session, "open session channel"));
    ...
    if (ssh_channel_request_pty_size(channel, "xterm-256color", cols, rows) != SSH_OK ||
        ...
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            ssh_error_str(session, "request pty/shell"));
        ssh_channel_free(channel);
        ssh_disconnect(session);
        ssh_free(session);
        ::close(native_fd);
    ...
    h->server_fingerprint = server_fingerprint;

    if (pthread_create(&h->reader, nullptr, io_reader, h) != 0) {
        throw_ssh_exception(env, "com/awkoo/terminal/core/SshConnectException",
                            "reader thread create failed");
        teardown(h, true);
        delete h;
        return 0;
    }
...
Java_com_awkoo_libterminal_ssh_SshProcess_sshResize(JNIEnv*, jclass, jlong handle, jint rows, jint cols) {
...
Java_com_awkoo_libterminal_ssh_SshProcess_sshGetServerFingerprint(JNIEnv* env, jclass, jlong handle) {
    native_ssh* h = reinterpret_cast<native_ssh*>(handle);
    if (h == nullptr || h->server_fingerprint.empty()) {
        return nullptr;
    }
    return env->NewStringUTF(h->server_fingerprint.c_str());
}

extern "C" JNIEXPORT jint JNICALL
Java_com_awkoo_libterminal_ssh_SshProcess_sshWait(JNIEnv*, jclass, jlong handle) {
    native_ssh* h = reinterpret_cast<native_ssh*>(handle);
    if (h == nullptr) {
        return -1;
    }
...
extern "C" JNIEXPORT void JNICALL
Java_com_awkoo_libterminal_ssh_SshProcess_sshKill(JNIEnv*, jclass, jlong handle) {
    native_ssh* h = reinterpret_cast<native_ssh*>(handle);
    if (h == nullptr) {
        return;
    }
...
extern "C" JNIEXPORT void JNICALL
Java_com_awkoo_libterminal_ssh_SshProcess_sshClose(JNIEnv*, jclass, jlong handle) {
    native_ssh* h = reinterpret_cast<native_ssh*>(handle);
    dispose(h);
}