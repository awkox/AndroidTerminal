...
/**
 * Unicode 15 字符宽度计算（wcwidth(3) 实现）。
 *
 * 基于 https://github.com/jquast/wcwidth 移植，不可打印字符返回 0。
 *
 * 注意：必须与以下仓库保持同步：
...
internal object WcWidth {
    ...
    private val ZERO_WIDTH = intArrayOf(
        ...
        require(it.size % 2 == 0) { "ZERO_WIDTH array length must be even" }
    }

    // https://github.com/jquast/wcwidth/blob/master/wcwidth/table_wide.py
    // from https://github.com/jquast/wcwidth/pull/64
    // at commit 1b9b6585b0080ea5cb88dc9815796505724793fe (2022-12-16):
    private val WIDE_EASTASIAN = intArrayOf(
        ...
        require(it.size % 2 == 0) { "WIDE_EASTASIAN array length must be even" }
    }

    private fun intable(table: IntArray, c: Int): Boolean {
        if (c < table[0]) return false

        var bot = 0
        var top = (table.size ushr 1) - 1
        while (top >= bot) {
    ...
    fun width(ucs: Int): Int = when {
        ...
        ucs in 0x2028..0x2029 ||
        ucs in 0x202A..0x202E ||
        ucs in 0x2060..0x2063 ||
        ucs < 32 ||
        ucs in 0x007F..<0x00A0 ||
        intable(ZERO_WIDTH, ucs) -> 0
    ...
    fun width(chars: CharArray, index: Int): Int {
        return chars.withCodePointAt(index) { codePoint, _ -> width(codePoint) }
    }

    /**
     * 统计 chars 数组中 [start] 到 [end)（不含）之间零宽字符（如组合字符）的数量。
     */
    fun zeroWidthCharsCount(chars: CharArray, start: Int, end: Int): Int {
        ...
        while (i < end && i < chars.size) {
            chars.withCodePointAt(i, chars.size) { cp, charCount ->
                if (width(cp) <= 0) count++
                i += charCount
            }
        }
