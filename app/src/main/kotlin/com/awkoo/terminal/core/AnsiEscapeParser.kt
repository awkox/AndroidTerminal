...
class AnsiEscapeParser(private val handler: TerminalActionHandler) {
    ...
    private fun doCsi(b: Int) {
        when (b.toChar()) {
            ...
            in '0'..'9', ';', ':' -> parseArg(b)
            else -> handler.onCsiCommand(ESC_CSI, b, mArgs, mArgIndex + 1, mArgsSubParamsBitSet)
        }
    }
    ...
    private fun doCsiQuestionMark(b: Int) {
        when (b.toChar()) {
            ...
            in '0'..'9', ';', ':' -> parseArg(b)
            else -> handler.onCsiCommand(ESC_CSI_QUESTIONMARK, b, mArgs, mArgIndex + 1, mArgsSubParamsBitSet)
        }
    }
    ...
    private fun doCsiBiggerThan(b: Int) {
        when (b.toChar()) {
            in '0'..'9', ';', ':' -> parseArg(b)
            else -> handler.onCsiCommand(ESC_CSI_BIGGERTHAN, b, mArgs, mArgIndex + 1, mArgsSubParamsBitSet)
        }
    }
    ...
    companion object {
        ...
        fun getArg(args: IntArray, index: Int, defaultValue: Int, treatZeroAsDefault: Boolean): Int {
