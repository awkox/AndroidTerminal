...
class TerminalRow(
    ...
    fun findStartOfColumn(column: Int): Int {
        ...
        while (true) {
            mText.withCodePointAt(currentCharIndex, mSpaceUsed) { codePoint, charCount ->
                val wcwidth = WcWidth.width(codePoint)
                if (wcwidth > 0) {
                    currentColumn += wcwidth
                    if (currentColumn == column) {
                        var newCharIndex = currentCharIndex + charCount
    ...
    fun getStyle(column: Int): TextStyle {
        return TextStyle(mStyle[column * 2])
    }

    fun getRawStyle(column: Int): Long = mStyle[column * 2]

    fun setRawStyle(column: Int, value: Long) { mStyle[column * 2] = value }

    fun getExtendedEffect(column: Int): Long = mStyle[column * 2 + 1]

    fun setExtendedEffect(column: Int, effect: Long) { mStyle[column * 2 + 1] = effect }
